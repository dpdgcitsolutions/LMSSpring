...
public class BookLoansDAO extends BaseDAO{
	...
	public void insertBookLoans(BookLoans bl) throws ClassNotFoundException, SQLException{
		...
		save("insert into tbl_book_loans (bookId, branchId, cardNo, dateOut, dueDate) values (?,?,?,?,?)", new Object[] {bl.getBookId(), bl.getBranchId(), bl.getCardNo(), dateOutStr, dueDateStr});
	}
...
//	}
	...
	public void updateDueDate(BookLoans bl) throws ClassNotFoundException, SQLException{
		...
		dueDate.add(Calendar.DATE, 7); // Adding 7 days
		String dueDateStr = sdf.format(dueDate.getTime());
		save("update tbl_book_loans set dueDate = ? where bookId = ? and branchId = ? and cardNo = ? and dateOut = ?", new Object[] {dueDateStr, bl.getBookId(), bl.getBranchId(), bl.getCardNo(), bl.getDateOut()});
	}
	...
	public void updateDateIn(BookLoans bl) throws ClassNotFoundException, SQLException{
		...
		save("update tbl_book_loans set dateIn = ? where bookId = ? and branchId = ? and cardNo = ? and dateOut = ?", new Object[] {dateInStr, bl.getBookId(), bl.getBranchId(), bl.getCardNo(), bl.getDateOut()});
	}
	
	public List<BookLoans> readAllByBranch(int branchId) throws ClassNotFoundException, SQLException {
		return read("select * from tbl_book_loans where branchId = ? and dateIn is null", new Object[]{branchId});
	}
